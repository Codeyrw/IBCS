...
class Encoder(torch.nn.Module):
    def __init__(self, gcn1,mlp1,mlp2,mlp3,mlp4,context,objects,edge_mlp):
        ...
        self.optimizer_shuf = Adam(self.shuf_mlp.parameters(), lr=0.01)
        self.optimizer_co = Adam(self.mlp_co.parameters(), lr=0.01)
        self.optimizer_o = Adam(self.mlp_o.parameters(), lr=0.01)
    def forward(self, data,flag=True):
        x, edge_index, batch = data.x, data.edge_index, data.batch
  
        z1, g1 = self.gcn1(x, edge_index, batch)

        IB_score = self.mlp_IB(z1.detach())
...
def train(encoder_model, data, optimizer_mlp1,optimizer_gcn1,optimizer_mlp2, optimizer_context,optimizer_objects,optimizer_edge,optimizer_mlp4,device):
    encoder_model.train()
    epoch_loss = 0
    CE = nn.CrossEntropyLoss()
    ...
    loss = 3*cls_loss0 + 2.5*cls_loss1 + 0.1*con_loss + 0.5*cls_loss2 +coc_loss  + 0.1*cls_loss3
    loss.backward()
    optimizer_mlp1.step()
    optimizer_gcn1.step()
    optimizer_mlp2.step()
...
def main():
    ...
    for fold, (train_idx, test_idx,
        ...
        gcn1 = GConv(gnn='GIN',input_dim=max(graphs.num_features, 1), hidden_dim=32, num_layers=2).to(device)
        ...
        context = GConv(gnn='GIN',input_dim=max(graphs.num_features, 1), hidden_dim=32, num_layers=2)
        objects = GConv(gnn='GIN',input_dim=max(graphs.num_features, 1), hidden_dim=32, num_layers=2)
        edge_att_mlp = FC_sub(input_dim=2*max(graphs.num_features, 1), hidden_dim=32, output_dim=2)

        encoder_model = Encoder(gcn1=gcn1,mlp1=mlp1,mlp2=mlp2,mlp3=mlp3,mlp4=mlp4,context= context,objects=objects,edge_mlp=edge_att_mlp).to(device)
        
        optimizer_gcn1  = Adam(gcn1.parameters(),lr=0.001)
        optimizer_mlp1 = Adam(mlp1.parameters(), lr=0.01)
        optimizer_mlp2 = Adam(mlp2.parameters(), lr=0.01)
        optimizer_mlp3 = Adam(mlp3.parameters(), lr=0.01)
        optimizer_mlp4 = Adam(mlp4.parameters(), lr=0.01)
        ...
        for epoch in range(1, args.epoch+1):
          ...
          for data in dataloader:
            ...
            for i in range(int(args.outer_steps)):
                loss = train(encoder_model, data, optimizer_mlp1,optimizer_gcn1,optimizer_mlp2,optimizer_context,optimizer_objects,optimizer_edge,optimizer_mlp4,device)
