...
class Encoder(torch.nn.Module):
    def __init__(self,n_class,x_dim):
        ...
        self.gcn1 = GConv(gnn='GIN',input_dim=max(x_dim, 1), hidden_dim=32, num_layers=2)

        self.mlp1 = FC(input_dim=32, hidden_dim=32, output_dim=27)

        self.context = GConv(gnn='GIN',input_dim=max(x_dim, 1), hidden_dim=32, num_layers=2)
        self.objects = GConv(gnn='GIN',input_dim=max(x_dim, 1), hidden_dim=32, num_layers=2)
        self.edge_mlp = FC_edge(input_dim=2*32, hidden_dim=32, output_dim=2)

        self.mlp_co = FC(input_dim=32, hidden_dim=32, output_dim=27)
        self.mlp_co_s = FC(input_dim=32, hidden_dim=32, output_dim=27)
        self.mlp_o = FC(input_dim=32, hidden_dim=32, output_dim=27)
        self.mlp_c = FC(input_dim=32, hidden_dim=32, output_dim=27)
...
def train(encoder_model,condis, data, optimizer_encoder_model,device):
    ...
    loss = 2.5*cls_loss1 + 3*cls_loss2   + 1*cls_loss4 + 2*cls_loss5  #+ 0.001*con_loss  
    loss.backward()
...
def arg_parse():
    ...
    parser.add_argument('--dataset', type=str, default='ogbg_molsider') 
...
def main():
    ...
    dataloader = loaders['train']  # batch size should be larger than 1
    val_dataloader = loaders['valid']
    test_dataloader = loaders['test']  # batch size should be larger than 1
