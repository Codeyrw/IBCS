...
class Encoder(torch.nn.Module):
    def __init__(self, gcn1,mlp1,mlp3,mlp4,mlp_edge,context,objects,bno,bnc):
        ...
        self.edge_mlp = mlp_edge
        self.mlp_co = FC(input_dim=32, hidden_dim=64, output_dim=2)
        self.mlp_o = FC(input_dim=32, hidden_dim=64, output_dim=2)
        self.mlp_co_s = FC(input_dim=32, hidden_dim=64, output_dim=2)
...
def train(encoder_model, data, optimizer_mlp1,optimizer_gcn1, optimizer_context,optimizer_objects,optimizer_edge,optimizer_mlp4,optimizer_bno,optimizer_bnc,device):
    encoder_model.train()
    epoch_loss = 0
    CE = nn.CrossEntropyLoss()
    ...
    loss = 3*cls_loss0 + 1*cls_loss1  + 0.1*con_loss  + 0.5*cls_loss2 + 1*cls_loss3
    loss.backward()
    optimizer_mlp1.step()
    optimizer_gcn1.step()
    optimizer_mlp4.step()
...
def main():
    ...
    for fold, (train_idx, test_idx,
        ...
        dataloader = DataLoader(train_dataset, batch_size=128,shuffle=True)  # batch size should be larger than 1

        val_dataloader = DataLoader(val_dataset, batch_size=128,shuffle=False)
        test_dataloader = DataLoader(test_dataset, batch_size=128,shuffle=False)  # batch size should be larger than 1
        gcn1 = GConv(gnn='GIN',input_dim=max(graphs.num_features, 1), hidden_dim=32, num_layers=2).to(device)

        mlp1 = FC(input_dim=32, hidden_dim=64, output_dim=graphs.num_classes)
        mlp3 = FC(input_dim=32, hidden_dim=64, output_dim=32)
        mlp4 = FC(input_dim=32, hidden_dim=64, output_dim=32)
        bnc = BatchNorm1d(32)
        bno= BatchNorm1d(32)
        ...
        edge_att_mlp = FC_edge(input_dim=2*32, hidden_dim=64, output_dim=2)
        encoder_model = Encoder(gcn1=gcn1,mlp1=mlp1,mlp3=mlp3,mlp4=mlp4,mlp_edge=edge_att_mlp,context= context,objects=objects,bno=bno,bnc=bnc).to(device)
        ...
        optimizer_edge = Adam(edge_att_mlp.parameters(),lr=0.001)
        ...
        for epoch in range(1, args.epoch+1):
          ...
          for data in dataloader:
            ...
            for i in range(int(args.outer_steps)):
                loss = train(encoder_model, data, optimizer_mlp1,optimizer_gcn1,optimizer_context,optimizer_objects,optimizer_edge,optimizer_mlp4,optimizer_bnc,optimizer_bno,device)
