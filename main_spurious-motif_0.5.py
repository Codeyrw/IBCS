...
class Encoder(torch.nn.Module):
    def __init__(self,n_class,x_dim):
        ...
        self.context = GConv(gnn='GIN',input_dim=max(x_dim, 1), hidden_dim=32, num_layers=self.n_class)
        self.objects = GConv(gnn='GIN',input_dim=max(x_dim, 1), hidden_dim=32, num_layers=self.n_class)
...
def train(encoder_model,condis, data, optimizer_encoder_model,device):
    ...
    loss = 3*cls_loss0 + 0.5*cls_loss1  + 0.1*con_loss + coc_loss  +2*cls_loss2 + 2.5*cls_loss3 
    loss.backward()
...
def arg_parse():
    ...
    parser.add_argument('--dataset', type=str, default='spmotif_0.5') 
