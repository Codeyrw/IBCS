...
from torch_geometric.nn import GCNConv, GATConv, SAGEConv,global_add_pool,global_mean_pool,BatchNorm
...
class GConv(nn.Module):
    def __init__(self, gnn,input_dim, hidden_dim, num_layers,deg_):
        ...
        if gnn == 'GCN':
            ...
            for i in range(num_layers):
                conv = PNAConvSimple(in_channels=hidden_dim*2, out_channels=hidden_dim, aggregators=aggregators,
                                 scalers=scalers, deg=deg_, post_layers=1)
    ...
    def forward(self, x, edge_index,batch,edge_attr=None,edge_atten=None):
        if self.gnn=='PNA':
            ...
            for i, (conv, batch_norm) in enumerate(zip(self.layers, self.batch_norms)):
...
class Encoder(torch.nn.Module):
    def __init__(self,n_class,x_dim,aux_info):
        super(Encoder, self).__init__()
        self.n_class =n_class 
        self.gcn1 = GConv(gnn='GIN',input_dim=max(x_dim, 1), hidden_dim=32, num_layers=2,deg_=aux_info['deg'])
       
        self.mlp1 = FC(input_dim=32, hidden_dim=32, output_dim=1)

        self.context = GConv(gnn='GIN',input_dim=max(x_dim, 1), hidden_dim=32, num_layers=2,deg_ =aux_info['deg'])
        self.objects = GConv(gnn='GIN',input_dim=max(x_dim, 1), hidden_dim=32, num_layers=2,deg_ =aux_info['deg'])
        self.edge_mlp = FC_edge(input_dim=2*32, hidden_dim=32, output_dim=2)

        self.mlp_co = FC(input_dim=32, hidden_dim=32, output_dim=1)
        self.mlp_co_s = FC(input_dim=32, hidden_dim=32, output_dim=1)
        self.mlp_o = FC(input_dim=32, hidden_dim=32, output_dim=1)
        self.mlp_c = FC(input_dim=32, hidden_dim=32, output_dim=1)
    ...
    def forward(self, data,flag=True):
        ...
        assignment =None
        row, col = edge_index
        edge_rep = torch.cat([z1[row], z1[col]], dim=-1)
        ...
        return h1,g1,h_M, g_M,assignment,h_co,h_res,edge_weight_o,assignment,h_shuf,y_shuf
...
def train(encoder_model,condis, data, optimizer_encoder_model,device):
    ...
    cls_loss1 = F.binary_cross_entropy_with_logits(h1,data.y.float())
    cls_loss2 = F.binary_cross_entropy_with_logits(h_M,data.y.float())
    cls_loss3= F.binary_cross_entropy_with_logits(h_co,data.y.float())
    uniform_target = torch.ones_like(h_res, dtype=torch.float).to(device) / h_res.shape[1]

    cls_loss4 = F.binary_cross_entropy_with_logits(h_res,data.y.float())
    cls_loss5 = F.binary_cross_entropy_with_logits(h_shuf,y_shuf.float())
    
    loss = 3*cls_loss1 + 3*cls_loss2   + 1.5*cls_loss4 +2*cls_loss5 + 0.1*con_loss 
    loss.backward()
...
def test_mnist(dataset_name,encoder_model, dataloader,multi_label,device):
    ...
    for data in dataloader:
        ...
        precision_at_k = get_precision_at_k(att.squeeze(1).data.cpu(), exp_labels.data.cpu(), k, data.batch, data.edge_index.data.cpu())
        h_Ms.append(h_M)
        atts.append(att.squeeze(1))
...
def arg_parse():
    ...
    parser.add_argument('--dataset', type=str, default='ogbg_molhiv') 
...
def main():
    ...
    encoder_model = Encoder(num_class,x_dim,aux_info).to(device)
    ...
    for epoch in range(1, args.epoch+1):
        ...
        print(f'(E): Test accuracy={clf_acc:.4f}, attroc={att_auroc:.4f},precision={precision:.4f},clf_roc={clf_roc:.4f}')
        ...
        print(epoch,best_roc)
    best_model.eval()
    if args.dataset=='mnist':
        with torch.no_grad():
            att_auroc, precision, clf_acc, clf_roc = test(args.dataset,best_model, test_dataloader,aux_info['multi_label'],device)

            print(f'(E): Test accuracy={clf_acc:.4f}, attroc={att_auroc:.4f},precision={precision:.4f},attroc={clf_roc:.4f}')
    else:
        with torch.no_grad():
            att_auroc, precision, clf_acc, clf_roc = test(args.dataset,best_model, test_dataloader,aux_info['multi_label'],device)
