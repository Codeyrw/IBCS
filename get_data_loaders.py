...
from datasets import SynGraphDataset, SPMotif, MNIST75sp, graph_sst2,Mutag
import json
...
def get_data_loaders(data_dir, dataset_name, batch_size, splits, random_state, mutag_x=False):
    ...
    if dataset_name == 'ba_2motifs':
        ...
        #print(loaders)
        train_set = dataset[split_idx['train']]
        np.save('mutag_train.npy',split_idx['train'])
        np.save('mutag_valid.npy',split_idx['valid'])
        np.save('mutag_test.npy',split_idx['test'])
        #json_str = json.dumps(split_idx)
        #with open('mutag_split.json', 'w') as json_file:
        ...
        print(test_set[-1].edge_label)
        print(test_set[-1].node_label)
    elif dataset_name == 'mnist':
        n_train_data, n_val_data = 20000, 5000
        train_val = MNIST75sp(osp.join(data_dir, 'mnist/'), mode='train')
        ...
        test_set = MNIST75sp(osp.join(data_dir, 'mnist/'), mode='test')
        loaders, test_set = get_loaders_and_test_set(batch_size, dataset_splits={'train': train_set, 'valid': valid_set, 'test': test_set})
        print('[INFO] Using default splits!')
    ...
    return loaders, test_set, x_dim,num_class, aux_info
