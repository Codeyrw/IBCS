...
class Encoder(torch.nn.Module):
    def __init__(self,n_class,x_dim):
        ...
        self.mlp_co = FC(input_dim=32, hidden_dim=32, output_dim=self.n_class)
        self.mlp_co_s = FC(input_dim=32, hidden_dim=32, output_dim=self.n_class)
    ...
    def forward(self, data,flag=True):
        ...
        g_res = global_max_pool(z_res, batch)
        g_M = global_max_pool(z_M, batch)
        ...
        return h1,g1,h_M, g_M,assignment,h_co,h_res,edge_weight_o,assignment,h_shuf,y_shuf,g_res
...
def train_M(encoder_model,condis, data, optimizer_condis,cur,device):
    ...
    h1,g1,h_M,g_M,ass,h_co,h_res,edge_weight_o,M,_,_,_= encoder_model(data,False)
...
def train(encoder_model,condis, data, optimizer_encoder_model,device):
    ...
    h1,g1,h_M,g_M,ass,h_co,h_res,edge_weight_o,M,h_shuf,y_shuf,_ = encoder_model(data,True)
    ...
    loss = 3*cls_loss0 + 3*cls_loss1 + 0.1*con_loss  + 2.5*cls_loss3 +1*cls_loss2
    loss.backward()
...
def test_mnist(dataset_name,encoder_model, dataloader,multi_label,device):
    ...
    for data in dataloader:
        ...
        h1, g1,h_M,g_M,ass,_,_,att,_,_,_,_ = encoder_model(data)
...
def val(dataset_name,encoder_model, dataloader, multi_label, device):
    ...
    for data in dataloader:
        data = process_data(data, False)
        data =data.to(device)
        h1, g1,h_M,g_M,ass,_,_,att,_,_,_,g_res= encoder_model(data)
        exp_labels = data.edge_label.data.cpu()
        cls_loss = CE(h_M,data.y).item()
        precision_at_k = get_precision_at_k(att.squeeze(1).data.cpu(), exp_labels.data.cpu(), k, data.batch, data.edge_index.data.cpu())
        loss+=cls_loss
        ...
        all_exp_labels.append(exp_labels)
        all_pre.append(precision_at_k)
    exp_labels, attss,g_M_all,g_res_all = torch.cat(all_exp_labels), torch.cat(atts),torch.cat(g_Ms),torch.cat(g_ress)
    np.save('g_cau.npy',g_M_all.detach().cpu().numpy())
    np.save('g_bias.npy',g_res_all.detach().cpu().numpy())
    
    clf_labels, clf_logits = torch.cat(y), torch.cat(h_Ms)
    np.save('y.npy',clf_labels.numpy())
    att_auroc, precision, clf_acc, clf_roc = get_eval_score(dataset_name,exp_labels, attss, precision_at_k, clf_labels, clf_logits, False,multi_label)
...
def main():
    ...
    for epoch in range(1, args.epoch+1):
        ...
        att_auroc, precision, clf_acc, clf_roc = test_mnist(args.dataset,encoder_model, test_dataloader,aux_info['multi_label'],device)
