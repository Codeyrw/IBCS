...
class Encoder(torch.nn.Module):
    def __init__(self, gcn1,mlp1,mlp2,mlp3,mlp4,edge_mlp,context,objects):
        ...
        self.shuf_mlp = FC(input_dim=32, hidden_dim=32, output_dim=2)
        self.mlp_co = FC(input_dim=32, hidden_dim=32, output_dim=2)
        self.mlp_o = FC(input_dim=32, hidden_dim=32, output_dim=2)
        self.optimizer_shuf = Adam(self.shuf_mlp.parameters(), lr=0.001)
        self.optimizer_co = Adam(self.mlp_co.parameters(), lr=0.001)
        self.optimizer_o = Adam(self.mlp_o.parameters(), lr=0.001)
    def forward(self, data,flag=True):
        ...
        assignment=None
        row, col = edge_index
        edge_rep = torch.cat([z1[row].detach(), z1[col].detach()], dim=-1)
        edge_att = F.softmax(self.edge_mlp(edge_rep), dim=-1)

        edge_weight_o = edge_att[:,0].unsqueeze(1)
        edge_weight_c = edge_att[:,1].unsqueeze(1)
        ...
        h_co_s = self.shuf_mlp(g_co_s)
        y_shuf= data.y[random_idx]

        proj_M = self.mlp3(g_M)
        proj1 = self.mlp3(g1) 
        return h1,g1,h_M, g_M,proj1,proj_M,assignment,h_co,h_res,h_co_s,y_shuf
...
def train(encoder_model, data, optimizer_mlp1,optimizer_gcn1,optimizer_mlp2, optimizer_context,optimizer_objects,optimizer_mlp4,optimizer_edge,device):
    encoder_model.train()
    epoch_loss = 0
    CE = nn.CrossEntropyLoss()
    ...
    encoder_model.optimizer_shuf.zero_grad()
    ...
    h1,g1,h_M,g_M,proj1,proj_M,ass,h_co,h_res,h_co_s,y_shuf = encoder_model(data,True)
    ...
    loss = 3*cls_loss0 + 0.5*cls_loss1 + 0.1*con_loss + 1*cls_loss2  + 2*cls_loss3   
    loss.backward()
    optimizer_mlp1.step()
    optimizer_gcn1.step()
    optimizer_mlp2.step()
    ...
    optimizer_edge.zero_grad()
    encoder_model.optimizer_shuf.step() 
...
def main():
    ...
    for fold, (train_idx, test_idx,
        ...
        dataloader = DataLoader(train_dataset, batch_size=64,shuffle=True)  # batch size should be larger than 1
        val_dataloader = DataLoader(val_dataset, batch_size=64,shuffle=False)
        test_dataloader = DataLoader(test_dataset, batch_size=64,shuffle=False)  # batch size should be larger than 1
        gcn1 = GConv(gnn='GIN',input_dim=max(graphs.num_features, 1), hidden_dim=32, num_layers=3).to(device)

        gcn_con = GConv(gnn='GIN',input_dim=max(graphs.num_features, 1), hidden_dim=32, num_layers=2).to(device)
        mlp1 = FC(input_dim=32, hidden_dim=32, output_dim=graphs.num_classes)
        mlp2 = FC_sub(input_dim=32, hidden_dim=32, output_dim=2)
        mlp3 = FC(input_dim=32, hidden_dim=32, output_dim=32)
        mlp4 = FC(input_dim=32, hidden_dim=32, output_dim=32)
        context = GConv(gnn='GIN',input_dim=max(graphs.num_features, 1), hidden_dim=32, num_layers=3)
        objects = GConv(gnn='GIN',input_dim=max(graphs.num_features, 1), hidden_dim=32, num_layers=3)
        edge_att_mlp = FC_edge(input_dim=2*32, hidden_dim=32, output_dim=2)

        encoder_model = Encoder(gcn1=gcn1,mlp1=mlp1,mlp2=mlp2,mlp3=mlp3,mlp4=mlp4,edge_mlp=edge_att_mlp,context= context,objects=objects).to(device)

        
        optimizer_gcn1  = Adam(gcn1.parameters(),lr=0.001)
        optimizer_mlp1 = Adam(mlp1.parameters(), lr=0.001)
        optimizer_mlp2 = Adam(mlp2.parameters(), lr=0.001)
        optimizer_mlp3 = Adam(mlp3.parameters(), lr=0.001)
        optimizer_mlp4 = Adam(mlp4.parameters(), lr=0.001)
        optimizer_edge = Adam(edge_att_mlp.parameters(), lr=0.01) 
        ...
        for epoch in range(1, args.epoch+1):
          ...
          for data in dataloader:
            ...
            for i in range(int(args.outer_steps)):
                loss = train(encoder_model, data, optimizer_mlp1,optimizer_gcn1,optimizer_mlp2,optimizer_context,optimizer_objects,optimizer_mlp4,optimizer_edge,device)
